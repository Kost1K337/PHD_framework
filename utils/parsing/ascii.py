...
def parse_wfrac_line(line, column_names, defaults, grid):
    """Get wellname and hydraulic fracturing from a single event file line.
    Expected format starts with: WELL 'DD.MM.YYYY' WFRP
    """
    vals = line.split()
    well = vals[0].strip("'\"")
    date = pd.to_datetime(vals[1], format='%d.%m.%Y', errors='coerce')
    vals = [v.upper() for v in vals]
    data = {'WELL': well + '_WFRAC', 'DATE': date}
    bias = 0
    for i, v in enumerate(vals[3:]):
        if '*' in v:
            k = int(v.split('*', 1)[0])
            for j in range(k):
                name = column_names[bias + i + j]
                if name in defaults:
                    value = defaults[name]
                    if isinstance(value, str) and 'grid' in value:
                        data[name] = grid.dimens[int(value.split('_')[-1])]
                    else:
                        data[name] = value
                else:
                    data[name] = np.nan
            ...
            name = column_names[bias + i]
            data[name] = [float(v) if v.replace('.', '', 1).isdigit() else v]
    return pd.DataFrame(data)
