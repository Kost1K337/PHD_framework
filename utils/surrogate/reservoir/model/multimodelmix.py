...
class MultiModelMix(mm.MultiModel):
    ...
    def view(self, time_step: int, hidden_cur: tc.Tensor):
        ...
        prod_bnd = self.poly.prod_bnd[time_step]
        prod_bnd = tc.from_numpy(prod_bnd).type(tc.float32)
        ...
        param = tc.cat((bhp_bnd, prod_bnd, status, wf), dim=1).to(device=self.device)
    ...
    def step(self, time_step: int, hidden_cur: tc.Tensor):
        ...
        bhp_bnd = self.poly.bhp_bnd[time_step + 1]
        bhp_bnd = tc.from_numpy(bhp_bnd).type(tc.float32)

        prod_bnd = self.poly.prod_bnd[time_step + 1]
        prod_bnd = tc.from_numpy(prod_bnd).type(tc.float32)
